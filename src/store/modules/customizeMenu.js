...
*/
const customizeMenuState = {
  state: {
    menu: [],
    isMenuActive: true, //是否是主菜单激活状态
  ...
  getters: {
    getActiveMenuItemSetting: (state) => {
      if (state.isMenuActive) {
        return state.menu[state.menuActiveIndex]
      } else {
        return state.menu[state.menuActiveIndex].sub_button.list[state.subMenuActiveIndex]
      }
    }
  },
  mutations: {
    ...
    SET_MENU_ACTIVE_STATE: (state, isMenuActive) => {
      state.isMenuActive = isMenuActive
    },
    SET_MENU_ACTIVE_INDEX: (state, menuActiveIndex) => {
...
export default customizeMenuState
